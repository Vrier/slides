...
- **2026-09-21: new opening for the Empirical lens.** The five-letter word game is gone
  ...
  *must*), with the question "where would you look to check each one". Answer slide:
  ...
  (1) to (33).
- **2026-09-21: hub renumbered; another round of deck edits; overlay PDF.** Hub, catalogue,
  overview, course plan and readings pages now follow the structure without Negation (wk 2
  Worlds, 3 Kratzer, 4 Attitudes, 5 Factivity, 6 Hyperintensionality). Deck: Today lists
  Łukasiewicz; readings without access notes; displacement answer trimmed; "Still open" row
  cut; *Epistemic and Deontic Flavour*; Alethic row on Flavours; *In view of…* prompt;
  *Trinity College Dublin* slide; photo credits off the slides (kept in comments); Kripke
  rows reworded, Hintikka row as Thomas worded it; Worked example, Your turn, Last week's
  swap and Limits cut; Irish English without Duality and Show of hands; final slide
  without "The handout:". 39 sections, examples (1) to (30). Beamer-style overlay PDF
  generated by capture (one page per revealed block), not in the repo.
- **2026-09-21: handout rebuilt as the companion to the deck's formal material.** Nine
  blocks, one per A4 page, in slide order, each margin note naming its slides: 0
  Intensions and propositions (the slide's four-world table incl. the tallest building and
  *is happy*); 1 The modal language (the deck's duality and scope sentences); 2 Frames,
  models, truth at a world (worked evaluation at w1 replaces the cut slides); 3 *Must* and
  *may* as quantifiers over worlds; 4 Validity (ants); 5 Frame conditions; 6 Flavours and
  their logics (six flavours as on the slide); 7 Substitution under necessity (Empire
  State pair, John's dogs, Ginsburg); 8 What you need to know + notation table mirroring
